...
fast-update step (prop/cpmc.py), blocks.block and run_qmc_energy. The one
difference is TABLE_MSD, which takes the minors det(C[s, :]) from reference
tables (below) instead of LAPACK: the same numbers to ~1e-10, and the reason this
script is ~8x faster per block than the redrawn one, whose determinant set changes
every step. TABLE_MSD = False is resampled_msd_cpmc.py's arithmetic exactly.
