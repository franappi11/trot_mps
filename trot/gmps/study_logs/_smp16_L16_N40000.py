...
def sampled_trial_energy(occ_a, occ_b, coeff, t_hop, u_int, nsite):
    ...
    i+1, so every matrix element is exactly -t with no sign bookkeeping. The
    ...
    for bit, other in ((1, 2), (2, 1)):          # alpha channel, then beta
        ...
        for i in range(nsite - 1):
            for src, dst in ((i, i + 1), (i + 1, i)):
                ...
                ok = j >= 0
                if ok.any():
                    e += -t_hop * float((c[sel[ok]] * c[j[ok]]).sum())
    return e / norm
...
if __name__ == "__main__":
    ...
    _t0 = time.perf_counter()
    e_trial_samp = sampled_trial_energy(occ_a, occ_b, coeff, t, U, L)
    print(f"   DMRG trial energy   (chi={CHI})     {float(E_dmrg_dav):.9f}")
    ...
    if RESULT_JSON:
        ...
        rec = dict(kind="sampling", tag=TAG, L=L, n_up=n_up, U=U, chi_trial=CHI,
                   ...
                   e_dmrg_trial=float(E_dmrg_dav), e_trial_sampled=float(e_trial_samp),
