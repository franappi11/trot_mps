...
def plan_channel(C, eps_occ=1e-10):
    ...
    return occ, np.array(Bs)



def channel_angles(C, plan):
    """
    ...
    angles = []
    for k, (B, vref) in enumerate(Bs):
        B = int(B)
        Uk = jnp.stack(rows[k : k + B])
        M = Uk @ Uk.T
        vr = jnp.asarray(vref)
        _, W = jnp.linalg.eigh(M)
...
def _gate_pair(A, B, th):
    ...
    t10 = A[:, 1, :] @ B[:, 0, :]
    t11 = A[:, 1, :] @ B[:, 1, :]
    return jnp.stack([jnp.stack([t00, c * t01 + s * t10], 1),
                     jnp.stack([-s * t01 + c * t10, t11], 1)], 1)
...
def mps_amp_all(ts, occ, chunk=1 << 20):
    ...
    filling), which is the MSD route being ejnponential, as advertised.
    """
    ns = occ.shape[0]
...
def plan_bonds(C, plan, chi_max=None, cutoff=0.0):
    ...
    for o in occ:
        qn.append(qn[-1] + int(o))
    angles, _ = channel_angles(np.asarray(C, float), plan, jnp=np)

    ks_all, discarded = [], 0.0
    for p, th in reversed(angles):
        T = _gate_pair(ts[p], ts[p + 1], th, jnp=np)
        Dl, _, _, Dr = T.shape
        M = T.reshape(Dl * 2, 2 * Dr)
...
def fast_sweep(ca, cb, rns, hs, w_floor):
    ...
    return ca, cb, ov_in, ov, jnp.ejnp(logw), nodes
...
def make_fast_prop_ops(ham_data, walker_kind, overlap_fn):
    ...
    def step(state, *, params, ham_data, trial_data, trial_ops,
        ...
        weights = weights * jnp.ejnp(prop_ctx.dt * state.pop_control_ene_shift)
...
if __name__ == "__main__":
    ...
    if RESULT_JSON:
        ...
        rec = dict(kind="mps", tag=TAG, L=L, n_up=n_up, U=U, chi_trial=CHI,
                chi_walker=CHI_WALKER, n_walkers=N_WALKERS,
