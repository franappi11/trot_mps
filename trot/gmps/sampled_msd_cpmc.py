...
Deliberately never builds the full CI expansion. Perfect sampling draws
n_1..n_N from |<n|Psi_T>|^2 ONCE; each draw carries its own exact amplitude, so the
trial is assembled from ONLY the determinants that came up, with COEFF choosing
their coefficients:

    "is"     |Psi~> = (1/N) sum_k |n_k> / Psi_T(n_k) = sum_{d drawn} hits_d / (N c_d) |d>
             notes.pdf eqs. (9)-(10), the estimator resampled_msd_cpmc.py redraws
             every step: E[<Psi~|phi>] = <Psi_T|phi> for every walker phi.
    "exact"  |Psi~> = sum_{d drawn} c_d |d>,  c_d = <d|Psi_T>: the plain truncation.
             Its mean over draws is sum_d P(d drawn) c_d |d>, not Psi_T.

Unbiased coefficients alone do NOT remove the bias of a FIXED trial: the constraint
is set by the one draw that is kept, and only redrawing averages over draws.
Measured at L=10 (exact -5.380619; chi=64 DMRG is exact there), N_SAMPLES = 100
(captured weight ~0.23), 200 walkers, 50 + 200 blocks, 4 seeds each:
    fixed, "is"                                  bias +0.041 +- 0.013
    fixed, "exact"                               bias +0.042 +- 0.018
    redrawn every step, "is" (resampled_msd_cpmc) bias +0.003 +- 0.008   (9 runs)
"is" does make runs more reproducible (spread 0.015 against 0.040 here). Its
variational energy is meaningless (+8 to +11 here): 1/c weights the rarest draws most.
With COEFF = "exact" this script reproduces its pre-trot-step version
(sampled_msd_cpmc.py.pre-trotfast.bak) to 1.8e-15 over 8 blocks at L=8, and
TABLE_MSD on and off agree to 5e-15 for either COEFF.

The CPMC is the same computation as resampled_msd_cpmc.py with the trial held
fixed: that script's site-basis MsdTrial ops and local energy, under trot's
fast-update step (prop/cpmc.py), blocks.block and run_qmc_energy. Both take the
minors det(C[s, :]) from reference tables when TABLE_MSD is on (the same numbers
as LAPACK to ~1e-10); here the references are fit to the one fixed set of strings,
there to pilot draws, since its set changes every step.
...
# the trial ops live in resampled_msd_cpmc.py, next to this file. trot/gmps is not
# part of the installed trot package, so look beside this file and one level up
# (study_run2.py runs copies of this script from study_logs/).
import os as _os, sys as _sys
_here = _os.path.dirname(_os.path.abspath(__file__))
_sys.path[:0] = [_here, _os.path.dirname(_here)]
from resampled_msd_cpmc import (MsdTrial, TRIAL_OPS, MEAS_OPS, energy_kernel,
                                pick_refs, _small_det)

# ============================================================== CONFIG
L, n_up, n_down = 16, 8, 8          # sites, electrons per spin
t, U            = 1.0, 4.0
...
N_SAMPLES       = 200_000           # determinants drawn (distinct ones are kept)
SAMPLE_SEED     = 1
SAMPLE_CHUNK    = 100_000           # draws per batch, to bound memory
COEFF           = "is"              # "is" = notes eq. (9), unbiased | "exact" = c_d on the drawn set
...
def sampled_trial_energy(occ_a, occ_b, coeff, h1_mat, u_int, nsite):
    ...
    the coefficients the trial carries (COEFF), so it is a strict variational state above the
...
def build_fixed_trial(occ_a, occ_b, coef, nsite, rdm1):
    """The drawn determinants as resampled_msd_cpmc's MsdTrial, one entry per
    DISTINCT determinant.

    Strings are indexed in np.unique order, the order pack_channel's reference
    tables are built in, so the table minors line up with `ia` / `ib`.
    """
    A, ia = np.unique(occ_a, axis=0, return_inverse=True)
    B, ib = np.unique(occ_b, axis=0, return_inverse=True)
    ia, ib = ia.reshape(-1), ib.reshape(-1)
    n = len(coef)
    code = np.zeros((n, nsite), np.uint8)
    rows = np.arange(n)[:, None]
    code[rows, occ_a] += 1
    code[rows, occ_b] += 2
    trial = MsdTrial(occ_a=jnp.asarray(A), occ_b=jnp.asarray(B), ia=jnp.asarray(ia),
                     ib=jnp.asarray(ib), coef=jnp.asarray(coef),
                     docc=jnp.asarray((code == 3).sum(1).astype(float)),
                     code=jnp.asarray(code), rdm1=rdm1,
                     n_str=jnp.asarray([len(A), len(B)]))
    return trial, A, B, ia, ib


def make_msd_ops(A, B, ia, ib, weight, nsite):
    """trot TrialOps / MeasOps for the fixed trial.

    With TABLE_MSD off these are resampled_msd_cpmc's, unchanged. With it on, the
    minors behind the overlap and behind the fast-update weights come from the
    reference tables; the fast-update ratio / update hooks and the Slater-Condon
    local energy (brute-force solve, once per block) are still that script's.
    `weight` (|c_d|^2 per determinant) only steers which strings become
    references. Returns (trial_ops, meas_ops, (pa, pb) or None).
    """
    if not TABLE_MSD:
        return TRIAL_OPS, MEAS_OPS, None
    # reference sets for the table form; nref ~ nA/200 is the measured optimum
    wA = np.zeros(len(A)); np.add.at(wA, ia, weight)
    wB = np.zeros(len(B)); np.add.at(wB, ib, weight)
    nra = N_REF if N_REF > 0 else int(np.clip(len(A) // 200, 1, 64))
    nrb = N_REF if N_REF > 0 else int(np.clip(len(B) // 200, 1, 64))
    pa = pack_channel(A, nsite, nra, wA, "greedy" if nra > 1 else "weight")
    pb = pack_channel(B, nsite, nrb, wB, "greedy" if nrb > 1 else "weight")
    print(f"   table MSD: {nra}/{nrb} references, mean excitation "
          f"{pa['k'].mean():.2f}/{pb['k'].mean():.2f} of nocc={A.shape[1]}")

    def det_weights(walker, trial):
        ca, cb = walker
        da = dets_from_table(ca, pa["refs"], pa["groups"], pa["nA"])
        db = dets_from_table(cb, pb["refs"], pb["groups"], pb["nA"])
        return trial.coef * da[trial.ia] * db[trial.ib]

    def overlap(walker, trial):
        return jnp.sum(det_weights(walker, trial))

    def calc_green(walker, trial):
        return {"w": det_weights(walker, trial), "code": trial.code}

    trial_ops = TRIAL_OPS._replace(overlap=overlap, calc_green=calc_green)
    meas_ops = MeasOps(overlap=overlap, kernels={k_energy: energy_kernel})
    return trial_ops, meas_ops, (pa, pb)
...
if __name__ == "__main__":
    assert COEFF in ("is", "exact"), f"COEFF must be 'is' or 'exact', got {COEFF!r}"
    ...
    # the coefficient each distinct determinant carries: "is" sums the N draws'
    # 1 / (N Psi_T(n_k)), duplicates included, so a determinant hit h times gets h / (N c)
    coef = hits / (N_SAMPLES * coeff) if COEFF == "is" else coeff
    print(f"   trial coefficients         : {COEFF}")

    _t0 = time.perf_counter()
    e_trial_samp = sampled_trial_energy(occ_a, occ_b, coef, h1, U, L)
    ...
    print(f"   sampled trial energy <H> variational {e_trial_samp:.9f}"
          f"   ({COEFF} coefficients, {time.perf_counter()-_t0:.1f} s)")

    rdm1 = jnp.asarray(np.stack([Ca @ Ca.T, Cb @ Cb.T]))       # seeds trot's walkers
    trial, A, B, ia, ib = build_fixed_trial(occ_a, occ_b, coef, L, rdm1)
    trial_ops, meas_ops, tables = make_msd_ops(A, B, ia, ib, coeff ** 2, L)
    print(f"   MSD trial: {len(coeff)} determinants, {len(A)} distinct alpha "
    ...
    params = dataclasses.replace(params, n_chunks=n_chunks)
    print(f"   peak walker-batched intermediate -> n_chunks = {n_chunks}")
    prop_ops = cpmc.make_prop_ops(ham, sys_.walker_kind, trial_ops)
    ...
    if tables is not None:
        ...
        _dr = np.abs(np.asarray(jax.vmap(lambda c: jnp.linalg.det(c[tables[0]["refs"]]))(_W[0])))
        _dm = np.abs(np.asarray(jax.vmap(lambda c: jnp.linalg.det(c[trial.occ_a]))(_W[0])))
        _ovf = np.asarray(jax.jit(jax.vmap(trial_ops.overlap, in_axes=(0, None)))(_W, trial))
        _ovb = np.asarray(jax.jit(jax.vmap(TRIAL_OPS.overlap, in_axes=(0, None)))(_W, trial))
    ...
    mean, err, be, bw = run_qmc_energy(
        sys=sys_, params=params, ham_data=ham, trial_data=trial,
    ...
    print(f"\nCPMC, sampled UHF-MSD trial ({COEFF})  {_f(mean)} +- {_f(err)}")
    ...
    if RESULT_JSON:
        ...
        rec = dict(kind="sampling", tag=TAG, L=L, n_up=n_up, U=U, chi_trial=CHI,
                   ...
                   n_alpha_strings=int(len(A)), n_beta_strings=int(len(B)),
                   weight=float((coeff**2).sum()), trial="uhf_msd", coeff=COEFF,
                   n_walkers=N_WALKERS, n_blocks=N_BLOCKS, n_eql=N_EQL,
                   n_prop=N_PROP, dt=DT, seed=SEED, e_hf=float(e_hf),
                   e_dmrg_trial=float(E_dmrg_T), e_dmrg_davidson=float(E_dmrg_dav),
                   e_trial_sampled=float(e_trial_samp), prop="trot_cpmc_fast",
                   table_msd=bool(TABLE_MSD), n_chunks=int(n_chunks),
                   ...
                   block_energies=np.asarray(be).real.tolist(),
                   block_weights=np.asarray(bw).real.tolist())
